...
export const sketchSnowWorld = () => {
  const sketch = (p: P5) => {
    let canvas: P5.Graphics;
    let movers: SnowParticle[];
    ...
    p.setup = () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        const y = p.random(
          -p.height * 0.5 - diameter,
          p.height * 0.5 + diameter,
        );
        const position = p.createVector(x, y);
        const speedX = p.random(1, 2.5);
        const speedY = p.random(4, 6.5);
        const velocity = p.createVector(speedX, speedY);
        movers.push(new SnowParticle(canvas, position, velocity, diameter));
      }
    };
